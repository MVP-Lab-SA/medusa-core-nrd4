...
export const CartDeleteItem = ({ item, fields }: CartDeleteItemProps) => {
  ...
  return (
    <Button
      ...
      className="text-city-muted hover:text-city-cyan transition-colors ml-2"
      variant="transparent"
      size="fit"
    >
      <Trash />
  ...
  return (
    <div className="flex items-center">
      <Button
        ...
        className={clsx(
          type === "compact" &&
            "text-city-muted hover:text-city-cyan transition-colors p-1 ml-2"
        )}
        variant="transparent"
        size="fit"
      >
      ...
      <span
        className={clsx(
          type === "compact"
            ? "text-sm text-city-white text-center px-3"
            : "text-center text-sm px-6 text-city-white"
        )}
      >
        {item.quantity}
      </span>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between">
          <div className="flex-1">
            <h4 className="text-base font-medium line-clamp-1 text-city-white">
              {item.product_title}
            </h4>
...
const DisplayCartLineItem = ({ item, cart, className }: CartLineItemProps) => {
  return (
    <div
      className={clsx(
        "flex items-center gap-4 py-3 border-b border-city-steel/30 last:border-b-0",
        className
      )}
    >
      <Thumbnail
      ...
      />
      <div className="flex-1">
        <p className="text-base font-semibold text-city-white">{item.product_title}</p>
        {item.variant_title && item.variant_title !== "Default Variant" && (
          <p className="text-sm text-city-muted">{item.variant_title}</p>
        )}
        <p className="text-sm text-city-muted">Quantity: {item.quantity}</p>
      </div>
  ...
  return (
    <div className="flex items-center gap-6 py-4">
      ...
      <div className="flex-1 min-w-0 flex flex-col gap-y-1">
        <span className="text-city-white text-base font-semibold">{item.product_title}</span>
        {item.variant_title && item.variant_title !== "Default Variant" && (
          <span className="text-city-muted text-sm">{item.variant_title}</span>
        )}
      </div>
...
export const CartSummary = ({ cart }: CartSummaryProps) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex justify-between text-sm">
        <span className="text-city-white font-medium">Total</span>
        <Price price={cart.total} currencyCode={cart.currency_code} className="text-city-white font-medium" />
      </div>
    </div>
  )
}
    ...
    <div className="flex flex-wrap gap-2">
          ...
          {cart.promotions.map((promotion) => (
            <Button key={promotion.code} variant="secondary" size="fit">
              ...
              <XMark
                ...
                className="ml-2 text-city-muted hover:text-city-cyan cursor-pointer"
              />
            </Button>
          ))}
        </div>
      ...
      {!showInput && (
        <Button
          ...
          variant="transparent"
          className="text-city-cyan p-0 underline hover:bg-transparent hover:text-city-cyan-light"
          size="fit"
        >
          Add promo code
        </Button>
...
export const CartEmpty = () => {
  ...
  return (
    <div className="text-center py-16 flex flex-col items-center justify-center gap-4">
      <h2 className="text-lg font-bold text-city-white">Your cart is empty</h2>
      <p className="text-city-muted text-base font-medium">Start by adding some products</p>
  ...
  return (
    <Drawer open={isOpen} onOpenChange={(open) => (open ? openCart() : closeCart())}>
      <DrawerTrigger asChild>
        <button className="text-city-gray hover:text-city-cyan h-full transition-colors flex items-center gap-2">
          <span>Cart</span>
          {itemCount > 0 && (
            <span className="bg-city-cyan text-city-dark text-xs font-semibold px-2 py-0.5 rounded-full">
              {itemCount}
            </span>
          )}
        </button>
      ...
      <DrawerContent className="flex flex-col bg-city-navy border-city-steel/30">
        <DrawerHeader className="border-city-steel/30">
          <DrawerTitle className="text-city-white">Shopping Cart</DrawerTitle>
        </DrawerHeader>

        {/* Empty Cart */}
        {(!cart || itemCount === 0) && (
          <div className="flex flex-col items-center justify-center flex-1 p-6">
            <span className="text-base font-medium text-city-muted mb-4">
              Your cart is empty
            </span>
        ...
        {cart && itemCount > 0 && (
          <>
            ...
            <DrawerFooter className="border-city-steel/30">
              <div className="flex items-center justify-between mb-4">
                <span className="text-base font-medium text-city-muted">Subtotal</span>
